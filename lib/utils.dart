...
class Utils {
  ...
  static Future<void> saveUser(
      User loggedInUser, String username, Function() finalState) async {
  ...
  static void streamTableAccount(
      Function(QueryDocumentSnapshot<Map<String, dynamic>> data) func,
    ...
    try {
      final snaps = store
          .collection('account')
          .where("uid_user", isEqualTo: auth.currentUser?.uid)
          .snapshots();
  ...
  static void getTableAccount(
    ...
    try {
      ...
      store
          .collection("account")
          .where(where ?? "uid_user", isEqualTo: value ?? auth.currentUser?.uid)
          .get()
          .then((value) {
  ...
  static void streamDebts(
      Function(List<QueryDocumentSnapshot<Map<String, dynamic>>> data) func,
      BuildContext context) async {
    final auth = FirebaseAuth.instance;
    final store = FirebaseFirestore.instance;
    ToastContext().init(context);
  ...
  static void getTableAnotherDebt(
      Function(List<QueryDocumentSnapshot<Map<String, dynamic>>> data) func,
      String uid,
      String username,
      BuildContext context) async {
  ...
  static void depositGetDept(
      QueryDocumentSnapshot<Map<String, dynamic>>? myAccount,
      int fixDeposit,
    ...
    Utils.getTableDebt((debts) async {
      try {
        ...
        if (debts.length > 0) {
          depositGetFriendAccount(
              debts, myAccount, fixDeposit, currentDeposit, context);
        } else {
          await myAccount?.reference
              .update({'deposit': (fixDeposit + currentDeposit)});
  ...
  static void depositGetFriendAccount(
      ...
      Utils.getTableAccount((myFriend) async {
        try {
          ...
          if (currentDebt > fixDeposit) {
            ...
            updateDebt(
                myFriend, myAccount, (currentDebt - fixDeposit), context);
            await myFriend?.reference
                .update({'deposit': (currentMyFriend + fixDeposit)});
            Navigator.pop(context);
  ...
  static void deleteDebt(
      QueryDocumentSnapshot<Map<String, dynamic>>? myFriend,
      QueryDocumentSnapshot<Map<String, dynamic>>? myAccount,
      BuildContext context) {
    try {
  ...
  static void transferMoney(
      String destination, String transactionValue, BuildContext context) async {
  ...
  static void myAnotherDebt(
      QueryDocumentSnapshot<Map<String, dynamic>>? myFriend,
      QueryDocumentSnapshot<Map<String, dynamic>>? myUser,
      int transaction,
      int currentDeposit,
      BuildContext context) {
    Utils.getTableAnotherDebt((userDebt) async {
          ...
          userDebt.forEach((element) async {
            ...
            if (currentDebt == transaction) {
              ...
              await element.reference
                  .update({'debt': currentDebt - transaction});
            }
          });
        }
      } catch (e) {
